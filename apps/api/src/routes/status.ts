...
import { getExecutionStatus } from "../services/status.js";
...
const plugin: FastifyPluginAsync<{ env: Env }> = async (app, opts) => {
  app.get<{ Params: { executionId: string } }>("/status/:executionId", async (request, reply) => {
    ...
    const row = await getExecutionStatus(request.params.executionId);
    if (!row) {
      return reply.status(404).send({
        code: "NOT_FOUND",
        message: "Execution not found.",
